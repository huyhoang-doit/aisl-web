  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
          ...
          <CardContent>
            ...
            <div className="mt-2 space-y-1">
              <div className="flex justify-between text-xs">
                ...
                <span className="font-medium text-blue-500">{(revenueStats?.rentalRevenue || 0).toLocaleString()} đ</span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-muted-foreground">Gửi hàng:</span>
                <span className="font-medium text-emerald-500">{(revenueStats?.deliveryRevenue || 0).toLocaleString()} đ</span>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            ...
            <div className="mt-2 grid grid-cols-2 gap-x-2 gap-y-1">
              <div className="flex flex-col">
                ...
                <span className="text-xs font-semibold text-blue-500">{orderStats?.activeOrders || 0}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-[10px] text-muted-foreground uppercase">Hoàn thành</span>
                <span className="text-xs font-semibold text-emerald-500">{orderStats?.completedOrders || 0}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-[10px] text-muted-foreground uppercase">Đã hủy</span>
                <span className="text-xs font-semibold text-red-500">{orderStats?.cancelledOrders || 0}</span>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardHeader className="p-4 sm:p-6 bg-muted/30 border-b">
            <div className="flex flex-col gap-4">
              <div className="flex justify-between items-start">
                <div>
                  <CardTitle className="text-lg sm:text-xl whitespace-nowrap">Diễn biến Doanh thu</CardTitle>
                  <CardDescription>Doanh thu hàng ngày</CardDescription>
                </div>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                <div className="flex items-center gap-1.5 shrink-0 bg-background p-1 rounded-md border shadow-sm">
                  <Input
                    type="date"
                ...
                <Button
                  variant="default"
                  size="sm"
                  className="h-8 gap-1.5 px-4 shadow-sm"
                ...
                <PieChart>
                  ...
                  >
                    {(lockerPieData.length ? lockerPieData : [{ name: "Trống", value: 1 }]).map((_entry, index) => (
                      <Cell key={`cell-${index}`} fill={lockerPieData.length ? COLORS[index % COLORS.length] : "#94a3b8"} strokeWidth={0} />
                    ))}
                  </Pie>
                  <Tooltip
                    contentStyle={{ borderRadius: "12px", border: "1px solid hsl(var(--border))", backgroundColor: "hsl(var(--popover))", color: "hsl(var(--popover-foreground))" }}
      ...
      <Card className="shadow-sm">
        <CardHeader className="flex flex-col xl:flex-row items-start xl:items-center justify-between gap-6 bg-muted/30 border-b p-4 sm:p-6">
          <div>
            <CardTitle className="text-xl font-bold tracking-tight">Chi tiết Hạ tầng &amp; Trạng thái Tủ</CardTitle>
            <CardDescription>Thống kê Cabinet và Locker theo từng khu vực (Click để xem chi tiết)</CardDescription>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex flex-col bg-card px-4 py-2 rounded-xl border shadow-sm transition-all hover:shadow-md">
              <div className="flex items-center gap-2 mb-1">
                <MapPin className="h-4 w-4 text-blue-500" />
                <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Địa điểm</span>
              </div>
              <div className="flex items-baseline gap-2">
                <span className="text-lg font-bold text-blue-500">{lockerStats?.totalLocations || 0}</span>
                <div className="flex gap-2 text-[10px]">
                  <span className="text-emerald-500">● {lockerStats?.activeLocations || 0} Hoạt động</span>
                  <span className="text-muted-foreground">● {lockerStats?.inactiveLocations || 0} Tạm ngưng</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col bg-card px-4 py-2 rounded-xl border shadow-sm transition-all hover:shadow-md">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Cụm tủ (Cabinet)</span>
              </div>
              <div className="flex items-baseline gap-2">
                <span className="text-lg font-bold text-orange-500">{lockerStats?.totalCabinets || 0}</span>
                <div className="flex gap-2 text-[10px]">
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/40 hover:bg-muted/40 border-b">
                ...
                <TableHead className="font-bold">Tên Vị trí / Cabinet</TableHead>
                <TableHead className="text-center font-bold">Trạng thái</TableHead>
                <TableHead className="text-center font-bold">Tổng số Tủ</TableHead>
                <TableHead className="text-center font-bold">Đang dùng</TableHead>
                <TableHead className="text-center font-bold">Còn trống</TableHead>
                <TableHead className="text-center font-bold">Ngoại tuyến</TableHead>
                <TableHead className="text-right font-bold px-6">Hiệu suất</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
                ...
                <React.Fragment key={loc.locationId}>
                  ...
                  >
                    <TableCell className="text-center">
                      {expandedLocations[loc.locationId] ? (
                        ...
                        <ChevronRight className="h-4 w-4 text-muted-foreground group-hover:text-blue-400 transition-all" />
                      )}
                    </TableCell>
                    <TableCell className="font-bold">{loc.locationName}</TableCell>
                    <TableCell className="text-center">
                    ...
                    <TableCell className="text-center font-semibold">{loc.totalLockers || 0}</TableCell>
                    <TableCell className="text-center text-blue-500 font-medium">{loc.occupiedLockers || 0}</TableCell>
                    <TableCell className="text-center text-emerald-500 font-medium">{loc.availableLockers || 0}</TableCell>
                  ...
                  {expandedLocations[loc.locationId] && loc.cabinetStats?.map((cab: any) => (
                    <React.Fragment key={cab.cabinetId}>
                      <TableRow
                        className="bg-muted/20 border-l-4 border-l-blue-400 hover:bg-muted/30 transition-colors cursor-pointer"
                      ...
                      >
                        ...
                        <TableCell className="pl-10 text-sm font-medium text-muted-foreground h-10">
                        ...
                        <TableCell className="text-center">
                          <Badge variant="outline" className={`text-[10px] h-5 ${cab.status === "ACTIVE" ? "border-emerald-500/50 text-emerald-500" : "text-muted-foreground"}`}>
                            {cab.status}
                          </Badge>
                        </TableCell>
                        <TableCell className="text-center text-sm text-muted-foreground">{cab.totalLockers}</TableCell>
                        <TableCell className="text-center text-sm text-blue-500/80">{cab.occupiedLockers}</TableCell>
                        <TableCell className="text-center text-sm text-emerald-500/80">{cab.availableLockers}</TableCell>
                        <TableCell className="text-center text-sm text-red-500/80">{cab.offlineLockers}</TableCell>
                        <TableCell className="text-right text-[10px] pr-8 text-muted-foreground font-mono">
                          {cab.totalLockers > 0 ? Math.round((cab.occupiedLockers / cab.totalLockers) * 100) : 0}%
                        </TableCell>
                      </TableRow>

                      {expandedCabinets[cab.cabinetId] && (
                        <TableRow className="bg-background border-l-4 border-l-blue-200">
                          <TableCell></TableCell>
